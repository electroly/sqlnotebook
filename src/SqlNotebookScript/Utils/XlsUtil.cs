...
public static class XlsUtil
{
    ...
    private sealed class Workbook : IWorkbook, IDisposable
    {
        ...
        )
        {
            ...
            lastColumnIndex = Math.Min(lastColumnIndex ?? int.MaxValue, _reader.FieldCount - 1);
            ...
            )
            {
                ...
                for (var j = firstColumnIndex; j <= (lastColumnIndex ?? (_reader.FieldCount - 1)); j++)
                {
    ...
    public static int? ColumnRefToIndex(object val)
