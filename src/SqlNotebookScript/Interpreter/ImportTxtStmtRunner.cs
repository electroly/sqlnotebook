...
public sealed class ImportTxtStmtRunner
{
    ...
    private ImportTxtStmtRunner(Notebook notebook, ScriptEnv env, ScriptRunner runner, Ast.ImportTxtStmt stmt)
    {
        ...
        _lineNumberColumnName =
            _stmt.LineNumberColumnName == null
                ? null
                : _runner.EvaluateIdentifierOrExpr(_stmt.LineNumberColumnName, _env);
        _textColumnName =
            _stmt.TextColumnName == null ? null : _runner.EvaluateIdentifierOrExpr(_stmt.TextColumnName, _env);
    ...
    private StreamReader NewReader(Stream fileStream)
    {
        ...
        var isUtf8 = (encoding?.CodePage ?? 0) switch
        {
            0 => true,
            65001 => true,
            _ => false,
        ...
        return _fileEncoding == null ? new(fileStream) : new(fileStream, _fileEncoding, false);
    }
    ...
    {
        _notebook.Execute(
            $"CREATE {(_temporaryTable ? "TEMPORARY" : "")} TABLE IF NOT EXISTS "
